...
# Invoke-NSUnsetNsparam

## SYNOPSIS
Clears one or more NetScaler nsparam resource properties.

## SYNTAX

```
Invoke-NSUnsetNsparam [-AdvancedAnalyticsStats] [-AftpallowrandomsourcePort] [-CIP] [-CIPHeader]
 [-CookieVersion] [-CrPortRange] [-ExclusivequotaMaxClient] [-Exclusivequotaspillover] [-FtpPortRange]
 [-GrantquotaMaxClient] [-Grantquotaspillover] [-HTTPPort] [-IcaPortS] [-InternalUserLogin] [-Ipttl] [-MaxConn]
 [-MaxReq] [-MgmtHTTPPort] [-MgmtHTTPSPort] [-PmtuMin] [-PmtuTimeout] [-ProxyProtocol] [-SecureCookie]
 [-SecureicaPortS] [-ServicePathingressvlan] [-TCPCip] [-Timezone] [-UseProxyPort] [[-Session] <PSObject>]
...
### EXAMPLE 1
```
Invoke-NSUnsetNsparam -AdvancedAnalyticsStats ENABLED -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetNsparam -AdvancedAnalyticsStats ENABLED -WhatIf
```
