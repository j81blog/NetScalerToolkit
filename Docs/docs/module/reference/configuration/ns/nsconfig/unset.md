...
## SYNOPSIS
Clears one or more NetScaler nsconfig resource properties.

## SYNTAX

```
Invoke-NSUnsetNSConfig [-CIP] [-CIPHeader] [-CookieVersion] [-CrPortRange] [-ExclusivequotaMaxClient]
 [-Exclusivequotaspillover] [-FtpPortRange] [-GrantquotaMaxClient] [-Grantquotaspillover] [-HTTPPort] [-Ifnum]
 [-Ipaddress] [-MaxConn] [-MaxReq] [-NetMask] [-Nsvlan] [-PmtuMin] [-PmtuTimeout] [-SecureCookie]
 [-Securemanagementtraffic] [-Tagged] [-Timezone] [[-Session] <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 1
```
Invoke-NSUnsetNSConfig -CIP ENABLED -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetNSConfig -CIP ENABLED -WhatIf
```
