...
## SYNOPSIS
Gets NetScaler rnat configuration.

## SYNTAX

### All (Default)
```
Invoke-NSGetRnat [-Filter <Hashtable>] [-ViewSummary] [-Session <PSObject>] [-IgnoreNotFound]
 [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### ByName
```
Invoke-NSGetRnat [-Name] <String> [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
 [-ThrowOnWarning] [<CommonParameters>]
```

### Count
```
Invoke-NSGetRnat [-Count] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning]
 [<CommonParameters>]
```
...
### EXAMPLE 2
```
Invoke-NSGetRnat -Name 'rnat_example'
```

### EXAMPLE 3
```
Invoke-NSGetRnat -Filter @{ servicetype = 'HTTP' }
```
