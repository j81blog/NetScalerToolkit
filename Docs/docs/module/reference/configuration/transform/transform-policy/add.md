...
```
Invoke-NSAddTransformPolicy [-Comment <String>] [-LogAction <String>] [-Name] <String> -ProfileName <String>
...
Can be changed after the URL Transformation policy is added.
The following requirement applies only to the Citrix ADC CLI: If the name includes one or more spaces, enclose the name in double or single quotation marks (for example, Ã‚"my transform policyÃ‚" or Ã‚'my transform policy).
Minimum length = 1
...
The following requirements apply only to the Citrix ADC CLI: * If the expression includes blank spaces, the entire expression must be enclosed in double quotation marks.
