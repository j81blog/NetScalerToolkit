...
## SYNOPSIS
Gets NetScaler mapbmr configuration.

## SYNTAX

### All (Default)
```
Invoke-NSGetMAPBMR [-Filter <Hashtable>] [-ViewSummary] [-Session <PSObject>] [-IgnoreNotFound]
 [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### ByName
```
Invoke-NSGetMAPBMR [-Name] <String> [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
 [-ThrowOnWarning] [<CommonParameters>]
```

### Count
```
Invoke-NSGetMAPBMR [-Count] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning]
 [<CommonParameters>]
```
...
### EXAMPLE 2
```
Invoke-NSGetMAPBMR -Name 'mapbmr_example'
```

### EXAMPLE 3
```
Invoke-NSGetMAPBMR -Filter @{ servicetype = 'HTTP' }
```
