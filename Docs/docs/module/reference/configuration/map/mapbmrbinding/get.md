...
## SYNOPSIS
Gets NetScaler mapbmr_binding configuration.

## SYNTAX

### All (Default)
```
Invoke-NSGetMAPBMRBinding [-Filter <Hashtable>] [-ViewSummary] [-Session <PSObject>] [-IgnoreNotFound]
 [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### ByName
```
Invoke-NSGetMAPBMRBinding [-Name] <String> [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
...
### EXAMPLE 2
```
Invoke-NSGetMAPBMRBinding -Name 'mapbmr_binding_example'
```

### EXAMPLE 3
```
Invoke-NSGetMAPBMRBinding -Filter @{ servicetype = 'HTTP' }
```
