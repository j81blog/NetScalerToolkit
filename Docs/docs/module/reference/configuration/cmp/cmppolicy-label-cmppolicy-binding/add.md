...
```
Invoke-NSAddCMPPolicyLabelCMPPolicyBinding [-GotoPriorityExpression <String>] [-Invoke <Boolean>]
...
After the invoked policies are evaluated, the flow returns to the policy with the next higher priority number in the original label.
