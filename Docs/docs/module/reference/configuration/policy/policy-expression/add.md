...
```
Invoke-NSAddPolicyExpression [-ClientsEcurityMessage <String>] [-Comment <String>] [-Name] <String>
 -Value <String> [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning] [-PassThru]
...
Message to display if the expression fails.
Allowed for classic end-point check expressions only.
Minimum length = 1
...
### -Comment
Any comments associated with the expression.
Displayed upon viewing the policy expression.
...
### -Name
Unique name for the expression.
Not case sensitive.
