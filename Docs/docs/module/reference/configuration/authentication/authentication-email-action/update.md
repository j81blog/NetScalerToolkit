...
## SYNOPSIS
Updates a NetScaler authenticationemailaction resource.

## SYNTAX

```
Invoke-NSUpdateAuthenticationEmailAction [-Content <String>] [-DefaultAuthenticationGroup <String>]
 [-EmailAddress <String>] [-Name] <String> [-Password <String>] [-ServerURL <String>] [-Timeout <Int32>]
 [-Type <String>] [-UserName <String>] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
...
### EXAMPLE 1
```
Invoke-NSUpdateAuthenticationEmailAction -Name 'example' -PassThru
```

### EXAMPLE 2
```
Invoke-NSUpdateAuthenticationEmailAction -Name 'example' -WhatIf
```
...
"$code" string within the content will be replaced with the actual one-time-code to be sent.
...
### -EmailAddress
An optional expression that yields user's email.
When not configured, user's default mail address would be used.
When configured, result of this expression is used as destination email address.
...
### -ServerURL
Address of the server that delivers the message.
It is fully qualified fqdn such as http(s):// or smtp(s):// for http and smtp protocols respectively.
...
Default type is SMTP.
Default value: SMTP Possible values = SMTP, ATHENA
