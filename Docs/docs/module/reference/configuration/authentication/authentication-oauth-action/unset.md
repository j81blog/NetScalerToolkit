...
## SYNOPSIS
Clears one or more NetScaler authenticationoauthaction resource properties.

## SYNTAX

```
Invoke-NSUnsetAuthenticationOAuthAction [-Name] <String> [-Allowedalgorithms] [-Attribute1] [-Attribute10]
 ...
 [-Attributes] [-Audience] [-Authentication] [-CertEndpoint] [-CertFilePath] [-DefaultAuthenticationGroup]
 [-Graphendpoint] [-Idtokendecryptendpoint] [-IntroSpecTURL] [-Intunedeviceidexpression] [-Issuer]
 [-MetaDataURL] [-OAuthMiscflags] [-OAuthType] [-Pkce] [-RefreshInterval] [-RequestAttribute] [-Resourceuri]
 [-Skewtime] [-TokenendpointauthMethod] [-UserInfoURL] [-UserNameField] [-Session <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 1
```
Invoke-NSUnsetAuthenticationOAuthAction -Name 'example' -Allowedalgorithms HS256 -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetAuthenticationOAuthAction -Name 'example' -Allowedalgorithms HS256 -WhatIf
```
