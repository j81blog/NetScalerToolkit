...
### All (Default)
```
Invoke-NSGetAuditMessageS [-Filter <Hashtable>] [-ViewSummary] [-Loglevel <String[]>] [-Numofmesgs <Int32>]
 [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### Count
```
Invoke-NSGetAuditMessageS [-Count] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
...
The following loglevels are valid: * ALL - All events.
