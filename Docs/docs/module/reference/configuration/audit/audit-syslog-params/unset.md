...
```
Invoke-NSUnsetAuditSyslogParams [-Acl] [-Alg] [-AppFlowExPort] [-ContentInSpecTionlog] [-DateFormAt]
 [-Denylistviolations] [-DNS] [-Logfacility] [-Loglevel] [-Lsn] [-ProtocolViolations] [-ServerIp] [-ServerPort]
 [-SSLInterception] [-StreamAnalytics] [-Subscriberlog] [-TCP] [-Timezone] [-URLFiltering]
 [-UserDefinedAuditLog] [[-Session] <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning]
...
* DDMMYYYY.
European style -date/month/year format.
...
* NONE - No events.
Possible values = ALL, EMERGENCY, ALERT, CRITICAL, ERROR, WARNING, NOTICE, INFORMATIONAL, NONE
...
Available settings function as follows: * GMT_TIME - Coordinated Universal Time.
...
Setting this parameter to YES causes audit to log user-configured message actions that meet the other logging criteria.
