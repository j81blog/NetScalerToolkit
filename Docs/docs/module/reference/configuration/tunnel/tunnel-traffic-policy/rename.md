...
## SYNOPSIS
Renames a NetScaler tunneltrafficpolicy resource.

## SYNTAX

```
Invoke-NSRenameTunnelTrafficPolicy [-Name] <String> -NewName <String> [-Session <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 1
```
Invoke-NSRenameTunnelTrafficPolicy -Name 'example' -NewName 'example' -PassThru
```

### EXAMPLE 2
```
Invoke-NSRenameTunnelTrafficPolicy -Name 'example' -NewName 'example' -WhatIf
```
...
### -NewName
New name for the tunnel traffic policy.
Must begin with an ASCII alphabetic or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at (@), e quals (=), and hyphen (-) characters.
