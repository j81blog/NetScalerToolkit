...
## SYNOPSIS
Renames a NetScaler nsacl resource.

## SYNTAX

```
Invoke-NSRenameACL [-AclName] <String> -NewName <String> [-Session <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 1
```
Invoke-NSRenameACL -AclName 'example' -NewName 'example' -PassThru
```

### EXAMPLE 2
```
Invoke-NSRenameACL -AclName 'example' -NewName 'example' -WhatIf
```
...
### -NewName
New name for the extended ACL rule.
Must begin with an ASCII alphabetic or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at (@), equals (=), and hyphen (-) characters.
Minimum length = 1
