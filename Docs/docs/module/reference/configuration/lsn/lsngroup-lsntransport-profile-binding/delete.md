...
```
Invoke-NSDeleteLSNGroupLSNTransportProfileBinding [-GroupName] <String> [-TransPortProfileName <String>]
...
Name of the LSN transport profile to bind to the specified LSN group.
Bind a profile for each protocol for which you want to specify settings.
By default, one LSN transport profile with default settings for TCP, UDP, and ICMP protocols is bound to an LSN group during its creation.
This profile is called a default transport.
