...
## SYNOPSIS
Clears one or more NetScaler vrid resource properties.

## SYNTAX

```
Invoke-NSUnsetVrid [-Id] <Int32> [-Ownernode] [-Preemption] [-Preemptiondelaytimer] [-Priority] [-Sharing]
 [-TrackifnumPriority] [-Tracking] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
...
### EXAMPLE 1
```
Invoke-NSUnsetVrid -Id 1 -Ownernode  -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetVrid -Id 1 -Ownernode  -WhatIf
```
...
If no owner is configured, owner node is displayed as ALL and one node is dynamically elected as the owner.
