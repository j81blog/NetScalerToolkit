...
## SYNOPSIS
Creates a NetScaler lbpolicylabel resource.

## SYNTAX

```
Invoke-NSAddLBPolicyLabel [-Comment <String>] [-LabelName] <String> [-PolicyLabelType <String>]
...
### EXAMPLE 1
```
Invoke-NSAddLBPolicyLabel -LabelName 'example' -PassThru
```

### EXAMPLE 2
```
Invoke-NSAddLBPolicyLabel -LabelName 'example' -WhatIf
```
...
### -PolicyLabelType
Protocols supported by the policylabel.
Available Types are : * HTTP - HTTP requests.
* DNS - DNS request.
...
Default value: HTTP Possible values = HTTP, OTHERTCP, SIP_UDP, SIP_TCP, MYSQL, MSSQL, ORACLE, NAT, DIAMETER, RADIUS, DNS, MQTT, QUIC_BRIDGE, HTTP_QUIC
