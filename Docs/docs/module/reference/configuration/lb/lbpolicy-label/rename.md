...
# Invoke-NSRenameLBPolicyLabel

## SYNOPSIS
Renames a NetScaler lbpolicylabel resource.

## SYNTAX

```
Invoke-NSRenameLBPolicyLabel [-LabelName] <String> -NewName <String> [-Session <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 1
```
Invoke-NSRenameLBPolicyLabel -LabelName 'example' -NewName 'example' -PassThru
```

### EXAMPLE 2
```
Invoke-NSRenameLBPolicyLabel -LabelName 'example' -NewName 'example' -WhatIf
```
...
### -NewName
New name for the LB policy label.
Must begin with a letter, number, or the underscore character (_), and must contain only letters, numbers, and the hyphen (-), period (.) hash (#), space ( ), at (@), equals (=), colon (:), and underscore characters.
Minimum length = 1
