...
# Invoke-NSUnsetLBProfile

## SYNOPSIS
Clears one or more NetScaler lbprofile resource properties.

## SYNTAX

```
Invoke-NSUnsetLBProfile [-LBProfileName] <String> [-ComputedadcCookieAttribute] [-CookiePassphrase] [-DBSLB]
 [-HTTPOnlyCookieFlag] [-LbHashAlgorithm] [-LbHashFingers] [-LiteraladcCookieAttribute] [-ProcessLocal]
 [-Proximityfromself] [-StoremqttClientIdandUserName] [-UseencryptedPersistenceCookie]
 [-UsesecuredPersistenceCookie] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
...
### EXAMPLE 1
```
Invoke-NSUnsetLBProfile -LBProfileName 'example' -ComputedadcCookieAttribute  -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetLBProfile -LBProfileName 'example' -ComputedadcCookieAttribute  -WhatIf
```
