...
## SYNOPSIS
Creates a NetScaler lbpolicylabel_lbpolicy_binding resource.

## SYNTAX

```
Invoke-NSAddLBPolicyLabelLBPolicyBinding [-GotoPriorityExpression <String>] [-Invoke <Boolean>]
...
### EXAMPLE 1
```
Invoke-NSAddLBPolicyLabelLBPolicyBinding -LabelName 'example' -PolicyName 'example' -Priority 1 -PassThru
```

### EXAMPLE 2
```
Invoke-NSAddLBPolicyLabelLBPolicyBinding -LabelName 'example' -PolicyName 'example' -Priority 1 -WhatIf
```
...
Available settings function as follows: * vserver - Invokes the unnamed policy label associated with the specified virtual server.
