...
## SYNOPSIS
Gets NetScaler lbglobal_lbpolicy_binding configuration.

## SYNTAX

### All (Default)
```
Invoke-NSGetLbglobalLBPolicyBinding [-Filter <Hashtable>] [-ViewSummary] [-Type <String>] [-Session <PSObject>]
 [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### Count
```
Invoke-NSGetLbglobalLBPolicyBinding [-Count] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
...
### EXAMPLE 2
```
Invoke-NSGetLbglobalLBPolicyBinding -Filter @{ servicetype = 'HTTP' }
```
