...
# Invoke-NSUnsetLBGroup

## SYNOPSIS
Clears one or more NetScaler lbgroup resource properties.

## SYNTAX

```
Invoke-NSUnsetLBGroup [-Name] <String> [-BackupPersistenceTimeout] [-CookieDomain] [-CookieName]
 [-MasterVServer] [-PersistenceBackup] [-PersistenceType] [-PersistMask] [-Rule] [-Timeout]
 [-UseVServerPersistency] [-V6PersistMaskLen] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
...
### EXAMPLE 1
```
Invoke-NSUnsetLBGroup -Name 'example' -BackupPersistenceTimeout  -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetLBGroup -Name 'example' -BackupPersistenceTimeout  -WhatIf
```
