...
# Invoke-NSRenameVPNVServer

## SYNOPSIS
Renames a NetScaler vpnvserver resource.

## SYNTAX

```
Invoke-NSRenameVPNVServer [-Name] <String> -NewName <String> [-Session <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 1
```
Invoke-NSRenameVPNVServer -Name 'example' -NewName 'example' -PassThru
```

### EXAMPLE 2
```
Invoke-NSRenameVPNVServer -Name 'example' -NewName 'example' -WhatIf
```
...
### -NewName
New name for the Citrix Gateway virtual server.
Must begin with an ASCII alphabetic or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at (@), equals (=), and hyphen (-) characters.
The following requirement applies only to the Citrix ADC CLI: If the name includes one or more spaces, enclose the name in double or single quotation marks (for example, "my server" or 'my server').
Minimum length = 1
