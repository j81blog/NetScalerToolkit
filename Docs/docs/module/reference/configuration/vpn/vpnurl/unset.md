...
## SYNOPSIS
Clears one or more NetScaler vpnurl resource properties.

## SYNTAX

```
Invoke-NSUnsetVPNURL [-URLName] <String> [-AppJson] [-ApplicationType] [-ClientLessaccess] [-Comment]
 [-IconURL] [-SAMLSSOProfile] [-SSOType] [-VServerName] [-Session <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 1
```
Invoke-NSUnsetVPNURL -URLName 'example' -AppJson  -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetVPNURL -URLName 'example' -AppJson  -WhatIf
```
