...
```
Invoke-NSAddBotProfileBlacklistBinding [-BotBindComment <String>] [-BotBlacklist <Boolean>]
 [-BotBlacklistAction <String[]>] [-BotBlacklistEnabled <String>] [-BotBlacklistType <String>]
 [-BotBlacklistValue <String>] [-LogMessage <String>] [-Name] <String> [-Session <PSObject>] [-IgnoreNotFound]
...
### -BotBlacklist
Blacklist binding.
Maximum 32 bindings can be configured per profile for Blacklist detection.
...
### -BotBlacklistAction
One or more actions to be taken if bot is detected based on this Blacklist binding.
Only LOG action can be combined with DROP or RESET action.
Default value: NONE Possible values = NONE, LOG, DROP, RESET, REDIRECT
