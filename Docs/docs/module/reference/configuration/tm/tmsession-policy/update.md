...
# Invoke-NSUpdateTMSessionPolicy
...
```
Invoke-NSUpdateTMSessionPolicy [-Action <String>] [-Name] <String> [-Rule <String>] [-Session <PSObject>]
...
Both classic and advance expressions are supported in default partition but only advance expressions in non-default partition.
