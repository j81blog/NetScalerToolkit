...
### All (Default)
```
Invoke-NSGetTmglobalTMSessionPolicyBinding [-Filter <Hashtable>] [-ViewSummary] [-Session <PSObject>]
 [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### Count
```
Invoke-NSGetTmglobalTMSessionPolicyBinding [-Count] [-Session <PSObject>] [-IgnoreNotFound]
