...
### All (Default)
```
Invoke-NSGetTmglobalTMTrafficPolicyBinding [-Filter <Hashtable>] [-ViewSummary] [-Session <PSObject>]
 [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### Count
```
Invoke-NSGetTmglobalTMTrafficPolicyBinding [-Count] [-Session <PSObject>] [-IgnoreNotFound]
