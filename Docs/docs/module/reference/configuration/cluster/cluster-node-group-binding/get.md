...
### All (Default)
```
Invoke-NSGetClusterNodeGroupBinding [-Filter <Hashtable>] [-ViewSummary] [-Session <PSObject>]
 [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### ByName
```
Invoke-NSGetClusterNodeGroupBinding [-Name] <String> [-Session <PSObject>] [-IgnoreNotFound]
...
If a name is not provided, information about all nodegroups is displayed.
Minimum length = 1
