...
# Invoke-NSUnsetChannel

## SYNOPSIS
Clears one or more NetScaler channel resource properties.

## SYNTAX

```
Invoke-NSUnsetChannel [-Id] <String> [-Bandwidthhigh] [-Bandwidthnormal] [-ConnDistr] [-Flowctl] [-Haheartbeat]
 [-HaMonItor] [-Ifalias] [-Linkredundancy] [-LrMinThroughput] [-Macdistr] [-Mode] [-Mtu] [-Speed] [-State]
 [-Tagall] [-Throughput] [-Trunk] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
...
### EXAMPLE 1
```
Invoke-NSUnsetChannel -Id 'example' -Bandwidthhigh  -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetChannel -Id 'example' -Bandwidthhigh  -WhatIf
```
