...
## SYNOPSIS
Gets NetScaler nshmackey configuration.

## SYNTAX

### All (Default)
```
Invoke-NSGetHMACKey [-Filter <Hashtable>] [-ViewSummary] [-Session <PSObject>] [-IgnoreNotFound]
 [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### ByName
```
Invoke-NSGetHMACKey [-Name] <String> [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
 [-ThrowOnWarning] [<CommonParameters>]
```

### Count
```
Invoke-NSGetHMACKey [-Count] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning]
 [<CommonParameters>]
```
...
### EXAMPLE 2
```
Invoke-NSGetHMACKey -Name 'nshmackey_example'
```

### EXAMPLE 3
```
Invoke-NSGetHMACKey -Filter @{ servicetype = 'HTTP' }
```
