...
## SYNOPSIS
Clears one or more NetScaler route resource properties.

## SYNTAX

```
Invoke-NSUnsetRoute [-Network] <String> [-Advertise] [-Cost1] [-Distance] [-Gateway] [-MonItor] [-Msr]
 [-NetMask] [-Protocol] [-TrafficDomain] [-Weight] [-Session <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 1
```
Invoke-NSUnsetRoute -Network 'example' -Advertise DISABLED -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetRoute -Network 'example' -Advertise DISABLED -WhatIf
```
