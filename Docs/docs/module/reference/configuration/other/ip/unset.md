...
## SYNOPSIS
Clears one or more NetScaler nsip resource properties.

## SYNTAX

```
Invoke-NSUnsetIP [-Ipaddress] <String> [-AdvertiseondefaultPartition] [-Arp] [-Arpowner] [-ArpResponse] [-Bgp]
 [-Decrementttl] [-Dynamicrouting] [-Ftp] [-Gui] [-HostRoute] [-HostRtgw] [-ICMP] [-ICMPResponse] [-Metric]
 [-Mgmtaccess] [-MpTCPAdvertise] [-NetMask] [-Networkroute] [-Ospf] [-Ospfarea] [-OspflsaType]
 [-OwnerDownResponse] [-Restrictaccess] [-Rip] [-SNMP] [-Ssh] [-Tag] [-TrafficDomain] [-Telnet] [-Vrid]
...
### EXAMPLE 1
```
Invoke-NSUnsetIP -Ipaddress 'example' -AdvertiseondefaultPartition ENABLED -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetIP -Ipaddress 'example' -AdvertiseondefaultPartition ENABLED -WhatIf
```
