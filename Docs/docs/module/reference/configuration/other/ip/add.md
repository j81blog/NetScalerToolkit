...
## SYNOPSIS
Creates a NetScaler nsip resource.

## SYNTAX

```
Invoke-NSAddIP [-AdvertiseondefaultPartition <String>] [-Arp <String>] [-Arpowner <Int32>]
 ...
 [-OwnerDownResponse <String>] [-Ownernode <Int32>] [-Restrictaccess <String>] [-Rip <String>] [-SNMP <String>]
 [-Ssh <String>] [-State <String>] [-Tag <Int32>] [-TrafficDomain <Int32>] [-Telnet <String>] [-Type <String>]
 [-Vrid <Int32>] [-VServer <String>] [-VServerRhilevel <String>] [-Session <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 1
```
Invoke-NSAddIP -Ipaddress 'example' -NetMask '255.255.255.255' -PassThru
```

### EXAMPLE 2
```
Invoke-NSAddIP -Ipaddress 'example' -NetMask '255.255.255.255' -WhatIf
```
...
### -Ownernode
The owner node in a Cluster for this IP address.
Owner node can vary from 0 to 31.
If ownernode is not specified then the IP is treated as Striped IP.
Default value: 255
