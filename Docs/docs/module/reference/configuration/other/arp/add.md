...
## SYNOPSIS
Creates a NetScaler arp resource.

## SYNTAX

```
Invoke-NSAddArp [-Ifnum <String>] [-Ipaddress] <String> -Mac <String> [-Ownernode <Int32>]
 [-TrafficDomain <Int32>] [-Vlan <Int32>] [-Vtep <String>] [-Vxlan <Int32>] [-Session <PSObject>]
...
### EXAMPLE 1
```
Invoke-NSAddArp -Ipaddress 'example' -Mac 'example' -PassThru
```

### EXAMPLE 2
```
Invoke-NSAddArp -Ipaddress 'example' -Mac 'example' -WhatIf
```

## PARAMETERS

### -Ifnum
Interface through which the network device is accessible.
Specify the interface in (slot/port) notation.
For example, 1/3.
