...
# Invoke-NSAddSimpleACL

## SYNOPSIS
Creates a NetScaler nssimpleacl resource.

## SYNTAX

```
Invoke-NSAddSimpleACL -AclAction <String> [-AclName] <String> [-DestPort <Int32>] [-Protocol <String>]
 -Srcip <String> [-TrafficDomain <Int32>] [-Ttl <Int32>] [-Session <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 1
```
Invoke-NSAddSimpleACL -AclName 'example' -AclAction DENY -Srcip 'example' -PassThru
```

### EXAMPLE 2
```
Invoke-NSAddSimpleACL -AclName 'example' -AclAction DENY -Srcip 'example' -WhatIf
```
...
Omitting the port number and protocol creates an all-ports and all protocols simple ACL rule, which matches any port and any protocol.
In that case, you cannot create another simple ACL rule specifying a specific port and the same source IPv4 address.
...
You must set this parameter if you have set the Destination Port parameter.
