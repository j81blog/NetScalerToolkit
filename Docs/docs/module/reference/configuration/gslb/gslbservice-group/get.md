...
### All (Default)
```
Invoke-NSGetGSLBServiceGroup [-Filter <Hashtable>] [-ViewSummary] [-IncludeMembers] [-Session <PSObject>]
 [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### ByName
```
Invoke-NSGetGSLBServiceGroup [-ServiceGroupName] <String> [-Session <PSObject>] [-IgnoreNotFound]
 [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### Count
```
Invoke-NSGetGSLBServiceGroup [-Count] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
...
Can be specified when no service group name is provided in the command.
In that case, the details displayed for each service group are identical to the details displayed when a service group name is provided, except that bound monitors are not displayed.
