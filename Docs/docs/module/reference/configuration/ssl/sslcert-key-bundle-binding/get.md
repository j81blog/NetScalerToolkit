...
## SYNOPSIS
Gets NetScaler sslcertkeybundle_binding configuration.

## SYNTAX

### All (Default)
```
Invoke-NSGetSSLCertKeyBundleBinding [-Filter <Hashtable>] [-ViewSummary] [-Session <PSObject>]
 [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### ByName
```
Invoke-NSGetSSLCertKeyBundleBinding [-CertKeyBundleName] <String> [-Session <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 2
```
Invoke-NSGetSSLCertKeyBundleBinding -CertKeyBundleName 'sslcertkeybundle_binding_example'
```

### EXAMPLE 3
```
Invoke-NSGetSSLCertKeyBundleBinding -Filter @{ servicetype = 'HTTP' }
```
