...
# Invoke-NSUnsetSSLOCSPResponder

## SYNOPSIS
Clears one or more NetScaler sslocspresponder resource properties.

## SYNTAX

```
Invoke-NSUnsetSSLOCSPResponder [-Name] <String> [-Batchingdelay] [-Batchingdepth] [-Cache] [-CacheTimeout]
 [-HTTPMethod] [-InsertClientCert] [-OCSPURLResolveTimeout] [-Producedattimeskew] [-ResponderCert]
 [-ResponseTimeout] [-SigningCert] [-TrustResponder] [-Usenonce] [-Session <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 1
```
Invoke-NSUnsetSSLOCSPResponder -Name 'example' -Batchingdelay  -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetSSLOCSPResponder -Name 'example' -Batchingdelay  -WhatIf
```
