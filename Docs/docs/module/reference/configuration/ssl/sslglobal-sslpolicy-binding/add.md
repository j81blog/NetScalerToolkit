...
## SYNOPSIS
Creates a NetScaler sslglobal_sslpolicy_binding resource.

## SYNTAX

```
Invoke-NSAddSSLGlobalSSLPolicyBinding [[-GotoPriorityExpression] <String>] [[-Invoke] <Boolean>]
...
### EXAMPLE 1
```
Invoke-NSAddSSLGlobalSSLPolicyBinding  -PassThru
```

### EXAMPLE 2
```
Invoke-NSAddSSLGlobalSSLPolicyBinding  -WhatIf
```
...
### -Invoke
Invoke policies bound to a virtual server, service, or policy label.
...
Specify virtual server for a policy label associated with a virtual server, or policy label for a user-defined policy label.
