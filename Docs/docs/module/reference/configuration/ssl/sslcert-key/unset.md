...
# Invoke-NSUnsetSSLCertKey

## SYNOPSIS
Clears one or more NetScaler sslcertkey resource properties.

## SYNTAX

```
Invoke-NSUnsetSSLCertKey [-CertKey] <String> [-DeleteCertKeyFilesOnRemoval] [-ExpiryMonItor]
 [-Notificationperiod] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning]
...
### EXAMPLE 1
```
Invoke-NSUnsetSSLCertKey -CertKey 'example' -DeleteCertKeyFilesOnRemoval NO -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetSSLCertKey -CertKey 'example' -DeleteCertKeyFilesOnRemoval NO -WhatIf
```
