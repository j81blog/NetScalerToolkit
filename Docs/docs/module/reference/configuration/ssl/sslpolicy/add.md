...
# Invoke-NSAddSSLPolicy

## SYNOPSIS
Creates a NetScaler sslpolicy resource.

## SYNTAX

```
Invoke-NSAddSSLPolicy [-Action <String>] [-Comment <String>] [-Name] <String> [-ReqAction <String>]
...
### EXAMPLE 1
```
Invoke-NSAddSSLPolicy -Name 'example' -Rule 'example' -PassThru
```

### EXAMPLE 2
```
Invoke-NSAddSSLPolicy -Name 'example' -Rule 'example' -WhatIf
```

## PARAMETERS

### -Action
Name of the built-in or user-defined action to perform on the request.
Available built-in actions are NOOP, RESET, DROP, CLIENTAUTH, NOCLIENTAUTH, INTERCEPT AND BYPASS.
...
Refer to 'add ssl action' command to add a new action.
Builtin actions like NOOP, RESET, DROP, CLIENTAUTH and NOCLIENTAUTH are also allowed.
Minimum length = 1
...
Possible values for data policies: NOOP, RESET, DROP and BYPASS.
