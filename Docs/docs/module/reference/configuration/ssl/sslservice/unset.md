...
# Invoke-NSUnsetSSLService

## SYNOPSIS
Clears one or more NetScaler sslservice resource properties.

## SYNTAX

```
Invoke-NSUnsetSSLService [-ServiceName] <String> [-CipherRedirect] [-CipherURL] [-ClientAuth] [-ClientCert]
 [-ComMonName] [-Dh] [-Dhcount] [-DhFile] [-DhKeyExpsizelimit] [-Dtls1] [-Dtls12] [-DtlsProfileName] [-Ersa]
 [-Ersacount] [-OCSPStapling] [-RedirectPortRewrite] [-Sendclosenotify] [-ServerAuth] [-Sessreuse]
 [-SessTimeout] [-Snienable] [-SSL2] [-SSL3] [-SSLClientLogs] [-SSLProfile] [-SSLRedirect] [-SSLV2Redirect]
 [-SSLV2URL] [-Strictsigdigestcheck] [-Tls1] [-Tls11] [-Tls12] [-Tls13] [-Session <PSObject>] [-IgnoreNotFound]
...
### EXAMPLE 1
```
Invoke-NSUnsetSSLService -ServiceName 'example' -CipherRedirect ENABLED -PassThru
```

### EXAMPLE 2
```
Invoke-NSUnsetSSLService -ServiceName 'example' -CipherRedirect ENABLED -WhatIf
```
