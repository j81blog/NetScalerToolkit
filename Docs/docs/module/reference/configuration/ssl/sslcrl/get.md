...
## SYNOPSIS
Gets NetScaler sslcrl configuration.

## SYNTAX

### All (Default)
```
Invoke-NSGetSSLCrl [-Filter <Hashtable>] [-ViewSummary] [-Session <PSObject>] [-IgnoreNotFound]
 [-ReturnNullOnNotFound] [-ThrowOnWarning] [<CommonParameters>]
```

### ByName
```
Invoke-NSGetSSLCrl [-CrlName] <String> [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound]
 [-ThrowOnWarning] [<CommonParameters>]
```

### Count
```
Invoke-NSGetSSLCrl [-Count] [-Session <PSObject>] [-IgnoreNotFound] [-ReturnNullOnNotFound] [-ThrowOnWarning]
 [<CommonParameters>]
```
...
### EXAMPLE 2
```
Invoke-NSGetSSLCrl -CrlName 'sslcrl_example'
```

### EXAMPLE 3
```
Invoke-NSGetSSLCrl -Filter @{ servicetype = 'HTTP' }
```
