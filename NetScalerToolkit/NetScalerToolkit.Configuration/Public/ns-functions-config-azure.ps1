# Generated NetScalerToolkit Configuration bundle for category 'azure'.

# region Invoke-NSAddAzureApplication
function Invoke-NSAddAzureApplication {
    <#
    .SYNOPSIS
        Creates a NetScaler azureapplication resource.
    .DESCRIPTION
        Configuration for Azure Application resource.
    .PARAMETER ClientId
        Application ID that is generated when an application is created in Azure Active Directory using either the Azure CLI or the Azure portal (GUI). Minimum length = 1 Maximum length = 223
    .PARAMETER ClientsEcret
        Password for the application configured in Azure Active Directory. The password is specified in the Azure CLI or generated in the Azure portal (GUI). Minimum length = 1 Maximum length = 223
    .PARAMETER Name
        Name for the application. Must begin with an ASCII alphanumeric or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at sign (@), equal sign (=), and hyphen (-) characters. Can be changed after the application is created.', CLI Users: If the name includes one or more spaces, enclose the name in double or single quotation marks (for example, "my application" or 'my application'). Minimum length = 1 Maximum length = 31
    .PARAMETER Tenantid
        ID of the directory inside Azure Active Directory in which the application was created. Minimum length = 1 Maximum length = 220
    .PARAMETER Tokenendpoint
        URL from where access token can be obtained. If the token end point is not specified, the default value is https://login.microsoftonline.com/<tenant id>. Minimum length = 1 Maximum length = 255
    .PARAMETER VaultResource
        Vault resource for which access token is granted. Example : vault.azure.net. Minimum length = 3 Maximum length = 254
    .PARAMETER Session
    ...
    .EXAMPLE
        Invoke-NSAddAzureApplication -Name 'example' -VaultResource 'example' -PassThru
    .EXAMPLE
        Invoke-NSAddAzureApplication -Name 'example' -VaultResource 'example' -WhatIf
    .OUTPUTS
    ...
    if ($PSBoundParameters.ContainsKey('ClientId')) { $properties['clientid'] = $ClientId }
    if ($PSBoundParameters.ContainsKey('ClientsEcret')) { $properties['clientsecret'] = $ClientsEcret }
    if ($PSBoundParameters.ContainsKey('Name')) { $properties['name'] = $Name }
    if ($PSBoundParameters.ContainsKey('Tenantid')) { $properties['tenantid'] = $Tenantid }
    if ($PSBoundParameters.ContainsKey('Tokenendpoint')) { $properties['tokenendpoint'] = $Tokenendpoint }
    if ($PSBoundParameters.ContainsKey('VaultResource')) { $properties['vaultresource'] = $VaultResource }
    ...
    if ($PSCmdlet.ShouldProcess("$Name", 'Create azureapplication')) {
        $response = Invoke-NSRestRequest @transportParameters -Session $Session -Method POST -Task config -Type azureapplication -Payload @{ azureapplication = $body } -RawResponse

        if ($PassThru) {
            return Invoke-NSGetAzureApplication -Name $Name -Session $Session
        }

        return $response
    }
}
# endregion Invoke-NSAddAzureApplication

# region Invoke-NSAddAzureKeyVault
function Invoke-NSAddAzureKeyVault {
    <#
    .SYNOPSIS
        Creates a NetScaler azurekeyvault resource.
    .DESCRIPTION
        Configuration for Azure Key Vault entity resource.
    .PARAMETER AzureApplication
        Name of the Azure Application object created on the ADC appliance. This object will be used for authentication with Azure Active Directory. Minimum length = 1 Maximum length = 63
    .PARAMETER AzureVaultName
        Name of the Key Vault configured in Azure cloud using either the Azure CLI or the Azure portal (GUI) with complete domain name. Example: Test.vault.azure.net. Minimum length = 3 Maximum length = 255
    .PARAMETER Name
        Name for the Key Vault. Must begin with an ASCII alphanumeric or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at sign (@), equal sign (=), and hyphen (-) characters. Can be changed after the Key Vault is created. CLI Users: If the name includes one or more spaces, enclose the name in double or single quotation marks (for example, "my keyvault" or 'my keyvault'). Minimum length = 1
    .PARAMETER Session
        The NetScaler session. If omitted, the current default session is used.
    .PARAMETER IgnoreNotFound
        Returns null instead of throwing for known NITRO not-found responses.
    ...
    .PARAMETER PassThru
        Returns the updated resource after the operation completes.
    .EXAMPLE
        Invoke-NSAddAzureKeyVault -Name 'example' -AzureApplication 'example' -PassThru
    .EXAMPLE
        Invoke-NSAddAzureKeyVault -Name 'example' -AzureApplication 'example' -WhatIf
    .OUTPUTS
        System.Management.Automation.PSCustomObject
    .NOTES
        Generated from NetScaler NITRO API metadata.
    ...
    $properties = [System.Collections.Generic.Dictionary[string, object]]::new([System.StringComparer]::Ordinal)
    if ($PSBoundParameters.ContainsKey('AzureApplication')) { $properties['azureapplication'] = $AzureApplication }
    if ($PSBoundParameters.ContainsKey('AzureVaultName')) { $properties['azurevaultname'] = $AzureVaultName }
    if ($PSBoundParameters.ContainsKey('Name')) { $properties['name'] = $Name }
    ...
    if ($PSCmdlet.ShouldProcess("$Name", 'Create azurekeyvault')) {
        $response = Invoke-NSRestRequest @transportParameters -Session $Session -Method POST -Task config -Type azurekeyvault -Payload @{ azurekeyvault = $body } -RawResponse

        if ($PassThru) {
            return Invoke-NSGetAzureKeyVault -Name $Name -Session $Session
        }

        return $response
    }
}
# endregion Invoke-NSAddAzureKeyVault

# region Invoke-NSDeleteAzureApplication
function Invoke-NSDeleteAzureApplication {
    <#
    .SYNOPSIS
        Removes a NetScaler azureapplication resource.
    .DESCRIPTION
        Configuration for Azure Application resource.
    .PARAMETER Name
        Name for the application. Must begin with an ASCII alphanumeric or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at sign (@), equal sign (=), and hyphen (-) characters. Can be changed after the application is created.', CLI Users: If the name includes one or more spaces, enclose the name in double or single quotation marks (for example, "my application" or 'my application'). Minimum length = 1 Maximum length = 31
    ...
    .PARAMETER ThrowOnWarning
        Treats NITRO warning responses as terminating errors.
    .EXAMPLE
        Invoke-NSDeleteAzureApplication -Name 'example' -Confirm:$false
    .EXAMPLE
        Invoke-NSGetAzureApplication -Name 'azureapplication_example' | Invoke-NSDeleteAzureApplication -Confirm:$false
    .OUTPUTS
        System.Management.Automation.PSCustomObject
    .NOTES
        Generated from NetScaler NITRO API metadata.
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess("$Name", 'Delete azureapplication')) {
            Invoke-NSRestRequest @transportParameters -Session $Session -Method DELETE -Task config -Type azureapplication -Resource $Name -RawResponse
        }
    }
}
# endregion Invoke-NSDeleteAzureApplication

# region Invoke-NSDeleteAzureKeyVault
function Invoke-NSDeleteAzureKeyVault {
    <#
    .SYNOPSIS
        Removes a NetScaler azurekeyvault resource.
    .DESCRIPTION
        Configuration for Azure Key Vault entity resource.
    .PARAMETER Name
        Name for the Key Vault. Must begin with an ASCII alphanumeric or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at sign (@), equal sign (=), and hyphen (-) characters. Can be changed after the Key Vault is created. CLI Users: If the name includes one or more spaces, enclose the name in double or single quotation marks (for example, "my keyvault" or 'my keyvault'). Minimum length = 1
    ...
    .PARAMETER ThrowOnWarning
        Treats NITRO warning responses as terminating errors.
    .EXAMPLE
        Invoke-NSDeleteAzureKeyVault -Name 'example' -Confirm:$false
    .EXAMPLE
        Invoke-NSGetAzureKeyVault -Name 'azurekeyvault_example' | Invoke-NSDeleteAzureKeyVault -Confirm:$false
    .OUTPUTS
        System.Management.Automation.PSCustomObject
    .NOTES
        Generated from NetScaler NITRO API metadata.
    ...
    process {
        ...
        if ($PSCmdlet.ShouldProcess("$Name", 'Delete azurekeyvault')) {
            Invoke-NSRestRequest @transportParameters -Session $Session -Method DELETE -Task config -Type azurekeyvault -Resource $Name -RawResponse
        }
    }
}
# endregion Invoke-NSDeleteAzureKeyVault

# region Invoke-NSGetAzureApplication
function Invoke-NSGetAzureApplication {
    <#
    .SYNOPSIS
        Gets NetScaler azureapplication configuration.
    .DESCRIPTION
        Configuration for Azure Application resource.
    .PARAMETER Name
        Name for the application. Must begin with an ASCII alphanumeric or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at sign (@), equal sign (=), and hyphen (-) characters. Can be changed after the application is created.', CLI Users: If the name includes one or more spaces, enclose the name in double or single quotation marks (for example, "my application" or 'my application'). Minimum length = 1 Maximum length = 31
    ...
    .PARAMETER ThrowOnWarning
        Treats NITRO warning responses as terminating errors.
    .EXAMPLE
        Invoke-NSGetAzureApplication
    .EXAMPLE
        Invoke-NSGetAzureApplication -Name 'azureapplication_example'
    .EXAMPLE
        Invoke-NSGetAzureApplication -Filter @{ servicetype = 'HTTP' }
    .OUTPUTS
        System.Management.Automation.PSCustomObject
    .NOTES
        Generated from NetScaler NITRO API metadata.
...
# endregion Invoke-NSGetAzureApplication

# region Invoke-NSGetAzureKeyVault
function Invoke-NSGetAzureKeyVault {
    <#
    .SYNOPSIS
        Gets NetScaler azurekeyvault configuration.
    .DESCRIPTION
        Configuration for Azure Key Vault entity resource.
    .PARAMETER Name
        Name for the Key Vault. Must begin with an ASCII alphanumeric or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at sign (@), equal sign (=), and hyphen (-) characters. Can be changed after the Key Vault is created. CLI Users: If the name includes one or more spaces, enclose the name in double or single quotation marks (for example, "my keyvault" or 'my keyvault'). Minimum length = 1
    ...
    .PARAMETER ThrowOnWarning
        Treats NITRO warning responses as terminating errors.
    .EXAMPLE
        Invoke-NSGetAzureKeyVault
    .EXAMPLE
        Invoke-NSGetAzureKeyVault -Name 'azurekeyvault_example'
    .EXAMPLE
        Invoke-NSGetAzureKeyVault -Filter @{ servicetype = 'HTTP' }
    .OUTPUTS
        System.Management.Automation.PSCustomObject
    .NOTES
        Generated from NetScaler NITRO API metadata.
...
# endregion Invoke-NSGetAzureKeyVault

# SIG # Begin signature block
# MII6AgYJKoZIhvcNAQcCoII58zCCOe8CAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC