function Set-NSACMECertificateChainLink {
<#
    .SYNOPSIS
        Links a NetScaler leaf certkey to provided CA chain certkeys.

    .DESCRIPTION
        Ensures provided intermediate/root certificates are present as NetScaler
        certkeys and links the leaf certificate to the first CA, then each CA to
        the next CA in the provided chain.

    .PARAMETER Session
        Existing NetScaler session returned by Connect-NSNode.

    .PARAMETER LeafCertKeyName
        Leaf certificate certkey name.

    .PARAMETER Certificate
        Posh-ACME certificate object.

    .PARAMETER IsProduction
        Indicates whether the ACME certificate is production.
...
#>
    ...
    if ($chainCertificates.Count -eq 0) {
        Write-NSACMECertificateLog Info 'Deploy' 'No provided chain certificates were found to link.'
        return @()
    }

    $existingCertKeys = @(Invoke-NSGetSSLCertKey -Session $Session -ErrorAction Stop)
    $chainCertKeys = @()
    foreach ($chainCertificate in $chainCertificates) {
        $chainCertKeys += Resolve-NSACMECertificateChainCertKey -Session $Session -Certificate $chainCertificate -ExistingCertKeys $existingCertKeys -IsProduction:$IsProduction
        if ($chainCertKeys[-1].Status -eq 'Added') {
            $existingCertKeys = @(Invoke-NSGetSSLCertKey -Session $Session -ErrorAction Stop)
        }
    }

    $previousCertKeyName = $LeafCertKeyName
    foreach ($chainCertKey in $chainCertKeys) {
        try {
            try { Invoke-NSUnlinkSSLCertKey -Session $Session -CertKey $previousCertKeyName -ErrorAction SilentlyContinue | Out-Null } catch {}
            Write-NSACMECertificateLog Info 'Deploy' "Linking certkey '$previousCertKeyName' to '$($chainCertKey.CertKeyName)'."
            Invoke-NSLinkSSLCertKey -Session $Session -CertKey $previousCertKeyName -LinkCertKeyName $chainCertKey.CertKeyName -ErrorAction Stop | Out-Null
        } catch {
            Write-NSACMECertificateLog Warning 'Deploy' "Could not link certkey '$previousCertKeyName' to '$($chainCertKey.CertKeyName)': $($_.Exception.Message)"
        }
        $previousCertKeyName = $chainCertKey.CertKeyName
    }

    return $chainCertKeys
}

# SIG # Begin signature block
# MII6AgYJKoZIhvcNAQcCoII58zCCOe8CAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCbv40pFqJdHhrL
# S7o1RWwnc1SxtyxrnHHn7OtTIMMaaaCCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQg4oY6
# ERvyXzEEc3AS7pl1C3hRARrE3TL5vYlhsnEhf/QwDQYJKoZIhvcNAQEBBQAEggGA
# f2B66mjVghSpzbuD6r2Q7AjSGJBj1yO1PXK2WcHKeKP5kscFUKBIAUNrXaE1uHTq
# 7lJ5rPICBgwBJ0DrdTzdowGysJ5cNDVRI3gE0TnkBfdfR2xp1EKfUbb8Dbv+Lqmo
