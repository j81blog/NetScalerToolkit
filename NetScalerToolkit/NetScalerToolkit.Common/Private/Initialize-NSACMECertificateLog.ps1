...
#>
    ...
    # -WhatIf:$false throughout: a WhatIf run must still write the log that describes what it
    # would do, otherwise the preview leaves no record.
    $logDir = Split-Path -Parent $Path
    if ($logDir) { New-Item -ItemType Directory -Path $logDir -Force -WhatIf:$false | Out-Null }
    if ((Test-Path -LiteralPath $Path) -and ((Get-Item -LiteralPath $Path).Length -gt ($MaxSizeMB * 1MB))) {
        ...
        Move-Item -LiteralPath $Path -Destination $archive -Force -WhatIf:$false
    }
    ...
    if ($LogType -eq 'jsonl') {
        ...
        Set-Content -LiteralPath $Path -Value ($header | ConvertTo-Json -Compress -Depth 6) -Encoding UTF8 -WhatIf:$false
    } else {
        $header = @(
            '================================================================================'
        ...
        Set-Content -LiteralPath $Path -Value $header -Encoding UTF8 -WhatIf:$false
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDhY3+alhQjZk+h
# 2YIJ1m7plBbk+zNYuj7lxwnaJoYRu6CCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQgxxXs
# fEfKgTvEvFKbsER1mpj6DLsuaGBL4pJFapbDfpowDQYJKoZIhvcNAQEBBQAEggGA
# ENim3ASSG+ouFuUu4+xdQfV9gud3JJS6hyPT4sWOZqc9QFXqeerejztvmCXHdqJv
# /sBDD7DoL73RnZB9evStMCpnQCtHsnPcoYZhtdLUR9y+tGTD4R2zKwOZDknDEgYT
# 2QIqPOagz+y9vCLYU+tFPJp8m3ddZXLh+ttkbi1iHhaZ6iyER3me/BaZEsiJVWO2
# 3qclIQLqHP1+YjC61pwupDkBSa0h6Km+DGy8zZmMRp94ORLeDF+T9R2LitSMwtjT
# K/nbZ53HpuBGLIrTiYSLovCA6d3mgdomIe9xL+eQTOlk/MrrxtizdbjKIYhN8AL4
# ScatjAnZYR4NT4cCd1n4HqGzs/yI9oSfHC/DLxEWzkVnR4E/e8AokxNGmKXWgrsv
# XD5S/rHOVBgxOHm+CnHl6dxlRy/l4URfwkD7PEo/xHpzI/P0aBU9lie0qgjib025
# iMYTzEoeWOhHZ0gSo3t4RclQDLIIcIrxjVj8nBXUynY0OkxnOzpn+AU+kWnTdWcI
# oYIUsjCCFK4GCisGAQQBgjcDAwExghSeMIIUmgYJKoZIhvcNAQcCoIIUizCCFIcC
# AQMxDzANBglghkgBZQMEAgEFADCCAWoGCyqGSIb3DQEJEAEEoIIBWQSCAVUwggFR
# AgEBBgorBgEEAYRZCgMBMDEwDQYJYIZIAWUDBAIBBQAEIF8T5mjRLiE7+kIgKi40
# vY8cL1KdNsePZa3IqAux3cJqAgZqddI03csYEzIwMjYwODE3MTg0MTE1LjYyMVow
...
# IgQgtNf5MDf1fWkqXyvcCWw2hFzOQg0qbbq9GIl873vvKBUwgd0GCyqGSIb3DQEJ
...
# bmcgQ0EgMjAyMAITMwAAAFtKtY1BMm3cdAAAAAAAWzAiBCDhhYzxTJUKX0s5TbHv
# LNZBfxKrUMw8+2pzArQV9hUi7DANBgkqhkiG9w0BAQsFAASCAgBQjqqBW4JJ+B1G
# M+6Qp0LJM/H06kjzZXyCPmcdyDqMxfXmvZ1S//YmQc3X7tm9cUYiQM7iuogbocN4
# XWSmRbcoKWIELFv4KybJFvNVfRvObt7QMU4WFC0wN0opDORyiQNdhkIcwG8jFxQt
# 9KfSb5/CQIzVowePjjca4EPa2nBiPu5Mnp40wPPztf6FuL26yLVstXF6fiJfQhl1
# YzVejzn7QyJyu8XYPVp4TZiSrCJUhG08izy/CsWIM6zjBQnmM/vXAO3LeyC3dEBM
# tsU14LlF533Xso+tIht9NvTm8rZZcIANEB8XzJK7cj8cyK4cnI+iQssq6AdzJSpF
# 71u8OmG4rCQbROx2I89rsNOoeumlpz0sghtt6dsoxPpVhFfHHpatktOerSyuUszy
# G8uVXmH+FtWxL0vX04SoUr+4eRUQtGp66W84JhgI9fPoLR3W77sMfQxo1lATk76s
# IHYZPilU+F2CK60SXIhg7XfAhgusONJyaR+SzZAJ3bXqEBKGaCfW0t+kAX/l5XTs
# 35AevGO0fylJOu6//KfD/XbOyysp4f/8ir/tXtn9K7Vvpk18unUI3N3IVQRoQPZR
# L5FWEAYZElUNA9L3a5GTODLiPh3U6jAjD9l5K9u4LbR1nKgrReIbcbeScRAZicE4
# uxjewvPj7pWJDirffuyCUTAO3ICkxg==
