function Normalize-NSACMECertificateRequest {
<#
    .SYNOPSIS
        Normalizes one ACME certificate request object.

    .DESCRIPTION
        Applies request-level defaults that are shared by direct and AutoRun
        certificate requests, and returns the de-duplicated domain list.

    .PARAMETER Request
        Certificate request object.

    .PARAMETER DefaultCertDir
        Default certificate output directory.

    .NOTES
        Function  : Normalize-NSACMECertificateRequest
        Author    : John Billekens
        Copyright : Copyright (c) John Billekens Consultancy
        Version   : 2026.0601.1600
#>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    Set-NSACMECertificateNoteProperty -InputObject $Request -Name ValidationMethod -Value $(if ($Request.ValidationMethod) { ([string]$Request.ValidationMethod).ToLowerInvariant() } else { 'http' })
    if ($Request.UseNetScalerDNS -or $Request.AlternateDNSValidationDomain) {
        Set-NSACMECertificateNoteProperty -InputObject $Request -Name ValidationMethod -Value 'dns'
    }
    if ($Request.CN -like '*`**' -or $Request.SANs -like '*`**') {
        Set-NSACMECertificateNoteProperty -InputObject $Request -Name ValidationMethod -Value 'dns'
    }
    Set-NSACMECertificateNoteProperty -InputObject $Request -Name UseLbVip -Value ([bool]$Request.UseLbVip)

    $domains = @($Request.CN)
    if ($Request.SANs) {
        $domains += ([string]$Request.SANs -split ',' | ForEach-Object { $_.Trim() } | Where-Object { $_ })
    }
    $domains = @($domains | Select-Object -Unique)

    if (-not $Request.FriendlyName) {
        Set-NSACMECertificateNoteProperty -InputObject $Request -Name FriendlyName -Value $Request.CN
    }
    if (-not $Request.CertDir) {
        Set-NSACMECertificateNoteProperty -InputObject $Request -Name CertDir -Value $DefaultCertDir
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBVOt44HGs1rFWd
# +yqaBewlUDyQ+1VXg9DoGyCBVWIaSaCCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQgpnA3
# 7QxkLko9waXzbp5+th+3fpMmR8ZZTWsawv2bSvcwDQYJKoZIhvcNAQEBBQAEggGA
# xBZB3Ri9xIaSLgZelVeLlyRGdA9wa68bRO7Zvx0LifbMfXSLmwywLYnRgXBk060G
# nvoA0GBunBoxDEfvPT9tW8HUg2OuhlWuRsjWXpL+uxAKwE7wT5v2FVCpb7b7b6P8
# S6Pr+q+EayG9F9v8gU2DKrzRU3FTB7u1CxmV0v8oVNW+u86rZWKLe6bfwBDNN18X
# FBphQb5+btjzBCr4AwBPfkQP/vCGlAOwwk2+Fsyp+zGn0M3CPxUsBHmxB34Hr8P6
# a5NiKejXFb/Whi2vsgqgCtCLwdTCXouna+TKQR3btdJ782LQR5AyO2TU1XBonBl1
# Zr+lRI4qIDscSZDnrRdrWO/JXkT1397v8E4zp2+NZK5nlwmC1qekuinopn6ywxzf
# ijb0NyePR4FTwZhQlpb94DHpCwv4ya11Jo2S+aG+q1Q4O9tsyB8iTm9tsqnVUBZM
# tFZtgpb9b3zawfITG8AF9sZWebQ2hDZBFovi7nElSEB0BnkJ1/d+F/1Kfktz/DcX
# oYIUsjCCFK4GCisGAQQBgjcDAwExghSeMIIUmgYJKoZIhvcNAQcCoIIUizCCFIcC
# AQMxDzANBglghkgBZQMEAgEFADCCAWoGCyqGSIb3DQEJEAEEoIIBWQSCAVUwggFR
# AgEBBgorBgEEAYRZCgMBMDEwDQYJYIZIAWUDBAIBBQAEIJKf+Hetw4uiAfGFvKOD
# 8WZBeBwIyDZ8YZ4QyHWXtRoCAgZqcNO0HxYYEzIwMjYwODAzMTg1NzQ3LjIxNFow
...
# IgQg0xl3lT5uTbXtMS3Dag1B/uCaTLLl8I+3/xLH8aBR+6owgd0GCyqGSIb3DQEJ
...
# syRAOrrxrLKyi2+/fLkFWcSxzDANBgkqhkiG9w0BAQsFAASCAgBrIlK0RpHoeiZb
# OwXapToEHTw0++U4LnL3MEK1iU2HblOw7RRyciG/MYJu8FO+HQjS3UCDGytCGvzv
# 39AxncsCP0v4pLktQVEjDr4PImM5trTbonAycP9Y+g4bPiym/bOXsyy4Nn1C2dQU
# TCSmMCME+UGZvC5fVsSA4nvv5KuWu5gotOfFdNDKvu1oo/i+prIZs8T1M9X9k+WY
# N4i403s2kAT2ivqaNQ0GCwMLQ68Av6Rnk43FwWkQmWtxVuxMKrE/JM1TMhRaklap
# 0mc3SLc29WJ3fEXmjWREqUNtavdA7LRVNwcyR685hXplIJCHETvNhxKX0dUqICSx
# 6et5palUoac9J87nQMGlkPgTB/eWH/9awjanljGGH7wI7e2z+bJ+VK9t9a3aXjrR
# MMZnTaKoSYTY/9l5fsAuwgueFZiRN+aOEY5zIiXWrAgQlkWV9iy8KTaQiStWjPJI
# nABHr9wxICvnqvG7gn15drUTxHMmgXXujmWlelOJfbTsEW3loPTc8yR1oFyc1edY
# u36tIdp4/mN6QXTiTramRk98U2OqWhio0gV7yq2y14YJgXW6o5RV+Tk4UB/LNSnd
# ZqS3T+8Kww5VHxu8fMcF5HQbbI5g9v0jwYBlbXbDwG4bREUd38RFw2WH9ip7S/fg
# WgaE+D31Lw8fzzJnyKg2JQqcjo9J5Q==
