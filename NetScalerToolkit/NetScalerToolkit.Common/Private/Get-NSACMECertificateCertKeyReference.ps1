function Get-NSACMECertificateCertKeyReference {
<#
    .SYNOPSIS
        Lists the NetScaler configuration references that hold an SSL certkey.

    .DESCRIPTION
        NITRO error 1541 refuses a certkey update and names every possible reason instead of the
        actual one. This walks the references NITRO exposes for a single certkey so the failure can
        report what really holds it.
    ...
    .OUTPUTS
        PSCustomObject with Type and Detail per reference.

    .NOTES
        Function  : Get-NSACMECertificateCertKeyReference
        Author    : John Billekens
        Copyright : Copyright (c) John Billekens Consultancy
        Version   : 2026.814.0100
#>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    $add = {
        param($Type, $Detail)
        $references.Add([PSCustomObject]@{ Type = $Type; Detail = $Detail }) | Out-Null
    }

    try {
        foreach ($binding in @(Invoke-NSGetVPNGlobalSSLCertKeyBinding -Session $Session -ReturnNullOnNotFound -ErrorAction Stop)) {
            if ($binding.certkeyname -eq $CertKeyName) { & $add 'VPN global' ('bind vpn global -certkeyName {0}' -f $CertKeyName) }
            if ($binding.cacert -eq $CertKeyName) { & $add 'VPN global CA' ('bind vpn global -cacert {0}' -f $CertKeyName) }
        }
    } catch {
        Write-NSACMECertificateLog Debug 'Deploy' "Could not read VPN global bindings for '$CertKeyName': $($_.Exception.Message)"
    }

    try {
        foreach ($link in @(Invoke-NSGetSSLCertLink -Session $Session -ReturnNullOnNotFound -ErrorAction Stop)) {
            if ($link.linkcertkeyname -eq $CertKeyName) { & $add 'CA of another certkey' ('{0} is linked to it' -f $link.certkeyname) }
        }
    } catch {
        Write-NSACMECertificateLog Debug 'Deploy' "Could not read SSL cert links for '$CertKeyName': $($_.Exception.Message)"
    }

    $bindingChecks = @(
        [PSCustomObject]@{ Command = 'Invoke-NSGetSSLCertKeySSLVServerBinding'; Type = 'SSL vServer'; Property = 'vservername' }
        [PSCustomObject]@{ Command = 'Invoke-NSGetSSLCertKeyServiceBinding'; Type = 'service'; Property = 'servicename' }
        [PSCustomObject]@{ Command = 'Invoke-NSGetSSLCertKeySSLProfileBinding'; Type = 'SSL profile'; Property = 'sslprofile' }
        [PSCustomObject]@{ Command = 'Invoke-NSGetSSLCertKeyCrldistributionBinding'; Type = 'CRL distribution point'; Property = 'crldistribution' }
        [PSCustomObject]@{ Command = 'Invoke-NSGetSSLCertKeySSLOCSPResponderBinding'; Type = 'OCSP responder'; Property = 'ocspresponder' }
    )
    foreach ($check in $bindingChecks) {
        try {
            foreach ($binding in @(& $check.Command -Session $Session -CertKey $CertKeyName -ReturnNullOnNotFound -ErrorAction Stop)) {
                if ($null -eq $binding) { continue }
                $name = [string]$binding.($check.Property)
                if ([string]::IsNullOrWhiteSpace($name)) { continue }
                & $add $check.Type $name
            }
        } catch {
            Write-NSACMECertificateLog Debug 'Deploy' "Reference check '$($check.Command)' failed for '$CertKeyName': $($_.Exception.Message)"
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBNPg/Sxt2oIQa9
# j4EmZDD77dd/ZQaflAMpQ0LJYjqf36CCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQ