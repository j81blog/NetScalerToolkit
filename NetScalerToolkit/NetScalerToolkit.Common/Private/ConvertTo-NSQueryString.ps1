function ConvertTo-NSQueryString {
    [CmdletBinding()]
    param(
        [Parameter()]
        [hashtable] $Query
    ...
    $pairs = foreach ($key in ($Query.Keys | Sort-Object)) {
        if ($null -ne $Query[$key] -and $Query[$key] -ne '') {
            if ($key -eq 'args') {
                $argumentParts = foreach ($argument in ([string] $Query[$key] -split ',')) {
                    $separatorIndex = $argument.IndexOf(':')
                    if ($separatorIndex -lt 0) {
                        [uri]::EscapeDataString($argument)
                        continue
                    }

                    $argumentName = $argument.Substring(0, $separatorIndex)
                    $argumentValue = $argument.Substring($separatorIndex + 1)
                    '{0}:{1}' -f [uri]::EscapeDataString($argumentName), [uri]::EscapeDataString($argumentValue)
                }

                '{0}={1}' -f [uri]::EscapeDataString($key), ($argumentParts -join ',')
                continue
            }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCASiSALaMVRH0Iz
# hpyu1SWqueedYB7T8GDdGXL2cDNiQKCCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQgIjCK
# B7N4jVdKU4ghX8fdYPkDJNe4QKCZ39ju7c9A2+4wDQYJKoZIhvcNAQEBBQAEggGA
# HR5MQrb28okTxzoVe7DsRLBT3Xk1nK+AauSkxBjVsUYieGt5n/zvSv0jpxYEM8jz
# s7e8k1f2WKjI4ehwB7bloIDay+81Hb8cXKWEHAua8RmJv7vFHMgf/oWYg2XFVw1Q
# nXYx2hEhKByi5cC4AxvqpuPob5hLhodtIOGza0RdkF1t8PRz+sDcy1LxeXB+Va/8
# 424JqDpxEmnFMiOEm1KhaY0lXHMLcvkrAQaqWF7Dzt0Yi+qLQocbyi1sdzPw0VCS
# H4QmmTJ/x/3rQmVqfHZekE8irZFq8sLgRUZKnciyG1EVMWQS+mK18nB9B0eGhdyO
# cVaRGvIISbMFsbDZtO2UluILrXX0TRlIv8Uz38rl6lz4Xb0bGdvzFB3ZjUrMb4SZ
# bPZCFeNZQILrQlHJ8nj2+okb0IiiaQbd38zmSIlUs4YL/Y8PaX7kG/RNmR800i1B
# 2akgDHnecoApV3uK1dpZT3JNy18w5o6GGZo3OvlxCkcdmuE/JpN6Va0TmPDpWaj6
# oYIUsjCCFK4GCisGAQQBgjcDAwExghSeMIIUmgYJKoZIhvcNAQcCoIIUizCCFIcC
# AQMxDzANBglghkgBZQMEAgEFADCCAWoGCyqGSIb3DQEJEAEEoIIBWQSCAVUwggFR
# AgEBBgorBgEEAYRZCgMBMDEwDQYJYIZIAWUDBAIBBQAEIN4pls1qFQwVA5C5PFj0
# 0GkLUKA6dvVjLUCNn8N/FN/MAgZqNTBfm8UYEzIwMjYwODAzMTI0OTE2LjUyOVow
...
# IgQg68qQju/FmPodlnoJnmTH/P732SqYcmBpZTTKFmDo2dEwgd0GCyqGSIb3DQEJ
...
# HHeWregoVp5960ewGO9kUZr6FDANBgkqhkiG9w0BAQsFAASCAgCHPZZyZMvZMooY
# X8saMHl+8CMvlnywK1pLTIsZkvN4tlK+uTzWqC9Hu0ZStO2rFnaSt/duR3J4umrJ
# WJnLPE0QtvJJJr6JHLuBKVQtARDH24MW1M2oou4Kf7JbBhumHyp8B7Fd2URx41Mj
# 8sNzylSiAw2i9M7N+qzhojTH5WjLM1aiRqLXGPceRuVH5xARleIEmKnO/LXI42Rd
# 4jq8BMRLY4LUGzeolBje6l7keTL+QsKRUrjSMLqIp1EAFVIp/+h+ytht6VlXkaee
# OsvyzKoxWAz4Q25t5I5ZZbeQyIMjOyFBYU4f3dPf64gx2q8jYWfV5q6Zu3dqNqzk
# 6zx603yeco/zj1W5To1fLycRwhcNqHj3shjCNicks76aJ+CldMorvJDD+dCdMWks
# F/Qe/jXtHkmlaUikKt/O1A5axix3e3Ev1KQ3RucXKTXKMghl6NQnHIOVSVXEid1I
# S0NyKGvPC5/66bYNM6MSrqlUHutQ9oBBUTmWh9GY1Y890e4EhvunWStIsLJ+vjMO
# 3aN318sXNq0TRhRK8NCsAnTuF7yDKKLKY6FP1yow5cCEtTkAUYasU9C8Ri0JxZOQ
# 4ygELKEPDoV4lZ2TP4m43XFMFXCoIdNkvSgL9TqHT5H6anrCjTl5p1ad7BNOTKFa
# tbav/KJKp4kOqqidVZFIKoPzRTr5iA==
