function Send-NSACMECertificateMail {
<#
    .SYNOPSIS
        Sends request-certificate completion mail.

    .DESCRIPTION
        Sends a concise completion mail with optional log attachment using the legacy
        SMTP parameters.
...
#>
    ...
    if (-not $Settings.SendMail) { return }
    foreach ($required in 'SMTPTo', 'SMTPFrom', 'SMTPServer') {
        if (-not $Settings.$required) {
            Write-NSACMECertificateLog Warning 'Mail' "Mail skipped. Missing setting '$required'."
            return
        }
    }

    # Send-MailMessage leaks the attachment file handle when the send fails, which locks
    # the log file for the rest of the run. SmtpClient lets us dispose it ourselves.
    $mailError = $null
    $message = New-Object System.Net.Mail.MailMessage
    $smtp = New-Object System.Net.Mail.SmtpClient($Settings.SMTPServer)
    try {
        $message.From = $Settings.SMTPFrom
        foreach ($to in @($Settings.SMTPTo)) { $message.To.Add($to) }
        $message.Subject = $Subject
        $message.Body = $Body
        $message.IsBodyHtml = $false
        $smtp.Port = if ($Settings.SMTPPort) { [int]$Settings.SMTPPort } else { 25 }
        $smtp.EnableSsl = [bool]$Settings.SMTPUseSSL
        if ($Settings.SMTPCredential -is [pscredential] -and $Settings.SMTPCredential -ne [pscredential]::Empty) {
            $smtp.Credentials = $Settings.SMTPCredential.GetNetworkCredential()
        }
        if ($Settings.LogAsAttachment -and $LogFile -and (Test-Path -LiteralPath $LogFile)) {
            $message.Attachments.Add((New-Object System.Net.Mail.Attachment -ArgumentList $LogFile))
        }
        $smtp.Send($message)
    } catch {
        $mailError = $_
    ...
    # Logged after disposal so the attachment no longer holds the log file.
    if ($mailError) {
        Write-NSACMECertificateLog Warning 'Mail' "Completion mail failed: $($mailError.Exception.Message)"
    } else {
        Write-NSACMECertificateLog Info 'Mail' "Completion mail sent to $(@($Settings.SMTPTo) -join ', ')."
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBNLAuB+qyJlZBJ
# FlE5pCDD5A/JsdZMkW1/2zu72kdvh6CCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQgpS8e
# JvBSzNGcG2azoLOcWL19ZQ7SxDZGY4DDKTOmZVowDQYJKoZIhvcNAQEBBQAEggGA
# AYz7yd5LqzVgU8SDe/iPsisAbbUppuZpsoyQ6F5VkHI9dHTpQGjyi1pSi6c0SQrH
# 6zn67kLjkt5gg+kSHNegXlzQJLgVStz39pzkWRIp/Bo6e/Weq506qZugPNrN93gR
# 3UYF9vCqHu0+kxfLphJtrOsvHmdJj4YUY3UXXhR+QnGZgjx5uY3uRHZPLnUqaxcG
