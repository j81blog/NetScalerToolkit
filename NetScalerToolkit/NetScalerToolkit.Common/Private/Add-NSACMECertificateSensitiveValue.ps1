function Add-NSACMECertificateSensitiveValue {
<#
    .SYNOPSIS
        Registers a sensitive value for ACME certificate log redaction.
#>
    [CmdletBinding()]
    param(
        ...
        [string]$Placeholder = '**SENSITIVE**'
    )

    process {
        if (-not $script:NSACMECertificateSensitiveValues) {
            $script:NSACMECertificateSensitiveValues = [System.Collections.Generic.List[object]]::new()
        }

        $text = $null
        if ($Value -is [securestring]) {
            $text = ConvertFrom-NSACMECertificateLegacySecret -Object $Value -AsClearText
        } elseif ($Value -is [pscredential]) {
            # [pscredential]::Empty can throw in Windows PowerShell when converted.
            try {
                $networkCredential = $Value.GetNetworkCredential()
            } catch {
                return
            }
            if ($networkCredential) { $text = $networkCredential.Password }
        } elseif ($null -ne $Value) {
            $text = [string]$Value
        }

        if ([string]::IsNullOrWhiteSpace($text) -or $text.Length -lt 3) { return }
        $existing = @($script:NSACMECertificateSensitiveValues | Where-Object {
                if ($_ -is [string]) { $_ -eq $text }
                else { $_.Value -eq $text }
            } | Select-Object -First 1)
        if (-not $existing) {
            $script:NSACMECertificateSensitiveValues.Add([pscustomobject]@{
                    Value       = $text
                    Placeholder = if ([string]::IsNullOrWhiteSpace($Placeholder)) { '**SENSITIVE**' } else { $Placeholder }
                })
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA2HVhJnD6XjfWb
# 43rK9z/mkRsnmt6gvGA4ZEBuqHVh7KCCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQgp38x
# A53U8IhB1LjfcoU12xz/fe63HgSqwRg1iOnS31wwDQYJKoZIhvcNAQEBBQAEggGA
# oH4r2jLX6MvWjDlfW7G3KYDQgbg1LE3/ypzk6UkLyNUZ7BDrvTito9utjpsx+keI
# skm4fiXok6Kk3dx8K2Jz48sDd+CHyg8jpCWJuMp1I9gEqWqNB+iunCfwE8mQTCXt
# HsZVF6mxvNETKxbHNXRmYr/axta1R+e0oMF6s6p5Rrr4SRF7hyMBk/QVttZCbJy7
# o/K+S2+Gx83dIl34Aek0iGOmSPo8oeng5ZambuXLSZ5DK6dQY87P/bxyCFBO93Od
# 4BcNk37zPJhog9/22sD+O7jsdyjf8pC7RvFjubKJnKwLQfH6NkapuGdve4UzBRwP
# o7Av4dr2w9PZwGvYudiAzvfWlCXLuY4F19ZwtEKmMliefLmmlPq8Tcs1KnKiuNDO
# sHZqIB5hqES26YPufrjsQXQYOnOd7oSHLcpdq2Cihzp6Ps9Widsvfz5wGnG7I78T
# 82JOgJUboZglZmgmfTSIjeFcCGDMHI0qTXVAWW+LtLUGIV4LZLUBip/lC5FSsQLA
# oYIUsjCCFK4GCisGAQQBgjcDAwExghSeMIIUmgYJKoZIhvcNAQcCoIIUizCCFIcC
# AQMxDzANBglghkgBZQMEAgEFADCCAWoGCyqGSIb3DQEJEAEEoIIBWQSCAVUwggFR
# AgEBBgorBgEEAYRZCgMBMDEwDQYJYIZIAWUDBAIBBQAEIJ/kI3ZN4tNmWY5T1+vx
# md2aPfjWz6/bb+qhK01U6RgLAgZqNWdtv10YEzIwMjYwODAzMTI0OTAzLjY0OFow
...
# IgQglkjOESY/743+4Gk66NFT0fBN8qKGCDPkTL8FIsl/3JAwgd0GCyqGSIb3DQEJ
...
# kTahOynpFQpsK7T7qt7engOZRjANBgkqhkiG9w0BAQsFAASCAgApH+MaajIXLJbH
# JO8j3D4rH7kaPYMT1fQlufpjVskSJ3jdoqRHGv4OQxo+8jqjXXVTsv0HNCgiekO9
# nTd2ACie0g8PAP10suYXDZ01KgsCYdKeEWJrIOBoKqovhYCEMcS07a9BH9Ddlo8y
# 13JRzeKEdmelzed8ldDyMP536XGnyvFNmzIQA3Js5/bUUQTeOYRL2UTeAUWusKh5
# FqZNgzyngjR/Y6TTgsp5cRkcU+qL20GDcYdYjK7eEG/VVPc6NwXXIHR6rprGpHE3
# h/90//bxLsJTyMo3yZohn2fSINwXbSHS3XQKv+sHW1gQB3yBuU0ygH9muZ0MJPpv
# 6UfBEA3cp/sS+DSeLjA7AmALTNa3zEkOMi9MqQrQnaxksGEQzakujfTJNZx6DGtr
# 7PZSqCPa7yUZ0Fup8dDD+fbVZCGfl8QAWr7tR3O59xMn3UoIlGR6zHpW3m9eB6lg
# NK3Zp711xJcRUtNsuUoIeIlxwslrCw2DPE5DtEJtHtcIMd452prHvTZrRkv2eXLn
# 2annUJJlRcBdBGhEXdnNGOGSGxQRwpACSSgaPFudy+o9J9o2hLy5Co2wIkMazQ/E
# 85SmvnXBxTow9UlyKIOOIkUJpBtMahvnPOiaG8oIvDyFVaL5Oi4e++pmwXQkxuH2
# FaWepY8GEH+N1Ej4s18z+YhiPspE5w==
