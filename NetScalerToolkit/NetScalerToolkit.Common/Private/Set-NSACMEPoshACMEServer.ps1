function Set-NSACMEPoshACMEServer {
<#
    .SYNOPSIS
        Sets the active Posh-ACME server while routing ToS output through the ACME logger.
#>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    $messages = @(& {
            Set-PAServer -DirectoryUrl $DirectoryUrl -ErrorAction Stop | Out-Null
        } 6>&1)

    $termsOfServiceUrl = $null
    foreach ($message in $messages) {
        $text = [string]$message
        if ($text -match 'Terms of Service here:\s*(?<Url>\S+)') {
            $termsOfServiceUrl = $Matches.Url
            break
        }
    }

    if (-not $termsOfServiceUrl) {
        try {
            $server = Get-PAServer -ErrorAction Stop
            if ($server -and $server.meta -and $server.meta.termsOfService) {
                $termsOfServiceUrl = [string]$server.meta.termsOfService
            }
        } catch {
            Write-NSACMECertificateLog Debug 'ACME' "Could not read ACME Terms of Service URL: $($_.Exception.Message)"
        }
    }

    if ($termsOfServiceUrl -and -not $script:NSACMETermsOfServiceLogged) {
        Write-NSACMECertificateLog Info 'ACME' "Terms of Service: $termsOfServiceUrl"
        Write-NSACMECertificateLog Info 'ACME' 'By running this command you agree with the ACME provider Terms of Service.'
        $script:NSACMETermsOfServiceLogged = $true
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC4o7yAltID+gfS
# ZZrPxe1AxNCdAmQWV87JaX+g8twi+6CCIiYwggXMMIIDtKADAgECAhBUmNLR1FsZ
...
# ADAZBgkqhkiG9w0BCQMxDAYKKwYBBAGCNwIBBDAvBgkqhkiG9w0BCQQxIgQgbzOc
# LVOKA2OVb4c5Dr6Swyzh58/u+F5IXI4jCiOM1CwwDQYJKoZIhvcNAQEBBQAEggGA
# I2SmbeBRlOK7rB8/49MMOPPUROpSmsuHL9aOm/ciHv5WqsBCc/X44qQI+K3oLi31
# qjAiVsA6AN1p7Tcgd1e6Hc8/JsZLRlfEiPcYqSs09VnZBt4Jym6PbDuSwZYYu+BL
# 3udUqmOkeR4udkCgUSpeafueamrKL/xZuu7a+HQPI74mEuPZ3vcxnJQo6GCsXTYm
# F4WT1oTfFfu9YVaArlI8WeoWjunakf+3S8ReMTvNV3kIvmWqz6Lz2sauVizYmq32
# b9OIDZrLKMS/jrffUje5cLSqfNPLfNXbRrVRgHcqmUhy0v8HZlypWv8Ey4O2EQgx
# nLoYQj7kPIe/HEzTDyLIcxbjSLdFWSiuXFfZQNHyzPs4PdclpdagUUI1RqaHxRSc
# h63UD7GFZwsvCtpTeEwrSi9w9gbr22d6ZWRXexCfFN5lrPozZaV6w/nSfXz27Tio
# P4XpcaHRSF/YMjj47aJt4b4DXNvqtbIKiVmsThRQlvoEsMfZsdMDDtsYFNYzPUiq
# oYIUsjCCFK4GCisGAQQBgjcDAwExghSeMIIUmgYJKoZIhvcNAQcCoIIUizCCFIcC
# AQMxDzANBglghkgBZQMEAgEFADCCAWoGCyqGSIb3DQEJEAEEoIIBWQSCAVUwggFR
# AgEBBgorBgEEAYRZCgMBMDEwDQYJYIZIAWUDBAIBBQAEIDSftNLsHPa6DyY72O7f
# dlIC5+FckpipOUbKDozghBqqAgZqNTBOPGYYEzIwMjYwNzI3MTMwOTI2LjM0N1ow
...
# IgQg49zgBbZn2M2sVc+kWQ9e73E4BDzfBIx2qJNVR7nWdaQwgd0GCyqGSIb3DQEJ
...
# C4pfvGQpmxVmt9dVg+iQDGdTljANBgkqhkiG9w0BAQsFAASCAgCrla4ROpcgO2AA
# 3YMt2Psdy3ZIYVxJQmh3o2SnUi1L2/TGBbDnKZDrVdidXJDQ9gDbdJrnWg8JQ7lO
# 4ExzxfH5io4IE1UHCW+fRje5L5KHeOZ89zBsgqltLajBlSabYMpENk+s+rpzcRaW
# 05f4/rEHhGeFxKE+uhkllaCdIxG8EFyWAv4ddU5ZsCpbWQqGQ3EHkCgEDG6I31gk
# cuapVGOyZ7Y9VyuMEbdX0nWDSbqYV68OZXkHZZTNZYwm4nbyYhiR+QIRv9pApgYH
# zhSuYFx4nZnMsPpj/L871wh/PU1VNO1F0LgRzXyyyyy0/ZNAVedW6WW9dtznxJ/v
# 9RwZOYG0wLNBL3RQ/5WkuLEIxr6t5BzA+/vAjxQja9a3onY+XeN7CZegpp+jELRp
# xJftWGwAVv2mofcASyquc6Ifq9pgaQY9Wv5TIQKd3QcqI3Q3Qq2WwLG+w/XY072W
# l/SqiqaqlBKN/a6AVY9WpkPIG4k/s3AG/ymkT7QOyLlqf3BsoKdQfOCs6yCplrOs
# B0t4zFyWb0Zo4KjSTUUb3ntzjs/GNaH3oSPu9Vcefscniqzi4itDZPIavbNj6153
# tkcp+Hqgp7qvQ4e+Q3dnlot4Hg367c6aT3FLdWotYCarYqJy8KUjxBNXmc2pkUme
# oNtvYgaqgGlTn2Jha3YIGqRxTOR5OA==
